...
pub fn build_ide(
    ...
    let mut children = vec![
        ...
        tag("dhEmi", &[], TagContent::Text(
            &format_datetime_nfe(&data.issued_at, &data.issuer.state_code),
        )),
        tag("tpNF", &[], TagContent::Text(
            &data.operation_type.unwrap_or(1).to_string(),
        )),
        tag("idDest", &[], TagContent::Text("1")),
        tag("cMunFG", &[], TagContent::Text(&data.issuer.city_code.0)),
        tag("tpImp", &[], TagContent::Text(
            data.print_format.as_deref().unwrap_or("1"),
        )),
        ...
        tag("finNFe", &[], TagContent::Text(
            &data.purpose_code.unwrap_or(1).to_string(),
        )),
        tag("indFinal", &[], TagContent::Text(
            data.consumer_type.as_deref().unwrap_or("0"),
        )),
        tag("indPres", &[], TagContent::Text(
            data.buyer_presence.as_deref().unwrap_or("0"),
        )),
        tag("indIntermed", &[], TagContent::Text(
            data.intermediary_indicator.as_deref().unwrap_or("0"),
        )),
        tag("procEmi", &[], TagContent::Text(
            data.emission_process.as_deref().unwrap_or("0"),
        )),
...
fn build_references(references: Option<&[ReferenceDoc]>) -> Vec<String> {
    let Some(refs) = references else { return vec![] };

    refs.iter().map(|r| match r {
        ReferenceDoc::Nfe { access_key } => {
            tag("NFref", &[], TagContent::Children(vec![
                tag("refNFe", &[], TagContent::Text(access_key)),
            ]))
        }
        ReferenceDoc::Nf { state_code, year_month, tax_id, model, series, number } => {
            tag("NFref", &[], TagContent::Children(vec![
                tag("refNF", &[], TagContent::Children(vec![
        ...
        ReferenceDoc::Nfp { state_code, year_month, tax_id, model, series, number } => {
            let tid = TaxId::new(tax_id);
            tag("NFref", &[], TagContent::Children(vec![
                tag("refNFP", &[], TagContent::Children(vec![
                    tag("cUF", &[], TagContent::Text(&state_code.0)),
                    tag("AAMM", &[], TagContent::Text(year_month)),
        ...
        ReferenceDoc::Cte { access_key } => {
            tag("NFref", &[], TagContent::Children(vec![
                tag("refCTe", &[], TagContent::Text(access_key)),
            ]))
        }
        ReferenceDoc::Ecf { model, ecf_number, coo_number } => {
            tag("NFref", &[], TagContent::Children(vec![
                tag("refECF", &[], TagContent::Children(vec![
                    tag("mod", &[], TagContent::Text(model)),
