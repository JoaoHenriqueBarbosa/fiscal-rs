...
<p align="center">
  <a href="https://fiscal-rs-docs.vercel.app/">Documentacao</a> &middot;
...
Port completo do [sped-nfe](https://github.com/nfephp-org/sped-nfe) (PHP) — a biblioteca fiscal brasileira mais usada (2.400+ stars) — reescrito em Rust com tipos algebricos, typestate pattern e 739+ testes.

Durante o desenvolvimento, [contribuimos 370 testes de volta ao sped-nfe](https://github.com/nfephp-org/sped-nfe/pull/1313) (PR #1313, mergeado), elevando a cobertura de **40% para 86,5%**.
...
O ecossistema Rust para documentos fiscais brasileiros e fragmentado e incompleto:
...
Containers Docker identicos: **1 CPU, 512 MB RAM**. [Metodologia completa →](https://fiscal-rs-docs.vercel.app/docs/benchmarks)

| Operacao | Rust | Bun/TS | PHP | Rust vs Bun | Rust vs PHP |
|----------|-----:|-------:|----:|:-----------:|:-----------:|
| **invoice_builder** | 27 us | 49 us | 427 us | **1.8x** | **16x** |
| **sign_xml** | 996 us | 1.9 ms | 3.4 ms | **1.9x** | **3.4x** |
| tag_nested_item | 3.0 us | 7.0 us | 9.7 us | **2.3x** | **3.2x** |
| serialize_icms00 | 829 ns | 1.0 us | 3.3 us | **1.2x** | **4.0x** |
| tag_simple_text | 122 ns | 224 ns | 1.0 us | **1.8x** | **8.5x** |
| escape_xml_clean | 40 ns | 111 ns | 124 ns | **2.8x** | **3.1x** |

Reproduza voce mesmo:
...
- **Ciclo completo NF-e/NFC-e**: geracao, assinatura, autorizacao, eventos, cancelamento, inutilizacao
- **Typestate pattern**: `InvoiceBuilder<Draft>` → `<Built>` → `<Signed>` — erros impossiveis em compilacao
...
- **Zero float drift**: valores monetarios em centavos `i64` (`10000` = R$ 100,00)
...
| Crate | Descricao | crates.io |
|-------|-----------|-----------|
| [`fiscal-core`](crates/fiscal-core) | Tipos, calculos, builder XML | [![](https://img.shields.io/crates/v/fiscal-core.svg)](https://crates.io/crates/fiscal-core) |
...
## Convencoes de valores

| Tipo | Armazenamento | Exemplo | Significado |
|------|---------------|---------|-------------|
| Dinheiro (vProd, vNF) | `Cents(i64)` | `Cents(1050)` | R$ 10,50 |
| Aliquota ICMS (pICMS) | `Rate(i64)` centesimos | `Rate(1800)` | 18,0000% |
| Aliquota PIS/COFINS | `Rate4(i64)` decimos de milesimo | `Rate4(16500)` | 1,6500% |

## Metodologia de porting

1. **Portar todos os testes 1:1** do PHP/TypeScript (739+ testes)
2. **Implementar codigo Rust** ate todos passarem
3. **Validar** contra ambiente de homologacao SEFAZ real
...
- [Fiscalidade](https://github.com/risoflora/fiscalidade) — Transmissao SEFAZ em Rust
